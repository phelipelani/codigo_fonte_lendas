...
export const GoalkeepersCleanSheetsCard = memo(({ goleiros }: { goleiros: any[] }) => {
  const top5 = goleiros?.slice(0, 5) || [];

  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }} className="rounded-2xl border border-purple-500/20 bg-surfaceElevated/50 p-4 md:p-5 flex flex-col">
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-2">
          <Shield size={16} className="text-purple-400" />
          <div>
            <h3 className="font-bold text-white text-xs uppercase tracking-wider">Goleiros</h3>
            <p className="text-[9px] text-purple-400 uppercase">Clean Sheets</p>
          </div>
        </div>
      </div>
        ...
        {top5.map((a: any, i: number) => {
          ...
          return (
            <div key={a.id} className="grid grid-cols-[16px_1fr_30px_30px] gap-2 items-center py-1.5">
              ...
              <span className="text-sm font-black text-purple-400 text-right">{a.clean_sheets}</span>
              <span className="text-[10px] font-bold text-purple-400/50 text-right">{perc}%</span>
            </div>
          );
        })}

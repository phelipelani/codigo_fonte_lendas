...
import { getPosicaoTipo, POSICAO_CORES } from '@/@types/cartolendas';
...
export function AbaEscalar({ campeonatoId }: { campeonatoId: number }) {
  ...
  const [filtro, setFiltro] = useState<'todos' | 'goleiro' | 'zagueiro' | 'linha'>('todos');
  ...
  const jogadoresFiltrados = useMemo(() => {
    if (!mercado) return [];
    let lista = [...mercado];
    if (filtro === 'goleiro') lista = lista.filter(j => j.posicao === 'goleiro');
    else if (filtro === 'zagueiro') lista = lista.filter(j => j.posicao !== 'goleiro' && j.joga_recuado);
    else if (filtro === 'linha') lista = lista.filter(j => j.posicao !== 'goleiro' && !j.joga_recuado);
          ...
          <div className="bg-white/[0.03] border border-white/10 rounded-2xl overflow-hidden">
            ...
            <div className="p-3 bg-white/[0.03] border-b border-white/10">
              ...
              <div className="flex gap-1.5 flex-wrap">
                {([
                  { key: 'todos', label: 'Todos', color: 'purple' },
                  { key: 'goleiro', label: 'GOL', color: 'blue' },
                  { key: 'zagueiro', label: 'ZAG', color: 'emerald' },
                  { key: 'linha', label: 'ATA/MEI', color: 'red' },
                ] as const).map(f => (
                  <button
                    key={f.key}
                    ...
                    className={cn(
                      ...
                      filtro === f.key
                        ? f.color === 'purple' ? 'bg-purple-600 text-white shadow-sm shadow-purple-900/40'
                        : f.color === 'blue' ? 'bg-blue-600 text-white shadow-sm shadow-blue-900/40'
                        : f.color === 'emerald' ? 'bg-emerald-600 text-white shadow-sm shadow-emerald-900/40'
              ...
              <div className="grid grid-cols-2 gap-2">
                {jogadoresFiltrados.map((j: any) => {
                  ...
                  return (
                    ...
                    >
                      {/* Topo colorido por posição */}
                      {(() => {
                        const pt = getPosicaoTipo(j.posicao, j.joga_recuado);
                        const topColors: Record<string, string> = {
                          goleiro: 'bg-gradient-to-r from-blue-400 to-blue-600',
                          zagueiro: 'bg-gradient-to-r from-emerald-400 to-emerald-600',
                          linha: 'bg-gradient-to-r from-red-400 to-red-600',
                        };
                        return <div className={cn('h-1', jaEsc ? 'bg-purple-500/50' : ehEu ? 'bg-gradient-to-r from-emerald-400 to-emerald-600' : topColors[pt])} />;
                      })()}
                      ...
                      <div className="p-2 flex flex-col items-center gap-1">
                        ...
                        {/* Posição colorida + Scouting Badge */}
                        <div className="flex items-center gap-1">
                          <PosicaoBadge tipo={getPosicaoTipo(j.posicao, j.joga_recuado)} className="text-[7px] px-1 py-0" />
                          {/* Scouting badges */}
                          {variacao > 1 && parseFloat(j.media_pontos ?? 0) > 3 && (
                          ...
                          {preco < 8 && parseFloat(j.media_pontos ?? 0) > 2 && (
                        ...
                        <div className="flex items-center gap-1.5">
                          <span className="text-[8px] text-purple-300/60 font-bold">{parseFloat(j.media_pontos ?? 0).toFixed(1)} pts</span>
                          {variacao !== 0 && (
                            <PontosDisplay valor={variacao} size="xs" showArrow className="text-[7px] px-1 py-0" />
                          )}
                        </div>

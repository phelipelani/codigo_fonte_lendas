...
export function AbaHistorico({ campeonatoId }: { campeonatoId: number }) {
  ...
  const StatCard = ({ title, jogadores, campo, sufixo = '', icone }: { title: string; jogadores: any[]; campo: string; sufixo?: string; icone: React.ReactNode }) => (
    <div className="bg-[#0f1c2e] border border-white/8 rounded-xl p-3">
      ...
      {jogadores?.length ? jogadores.map((j: any, i: number) => (
        <div key={j.id} className="flex items-center gap-2 py-1.5 border-b border-white/5 last:border-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[10px] text-white/30">{j.posicao}</p>
          </div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2">
        <button onClick={() => setSubAba('meu')} className={cn('flex-1 py-2 rounded-lg text-sm font-bold transition-all', subAba === 'meu' ? 'bg-purple-600 text-white' : 'bg-white/5 text-white/50 hover:bg-white/10')}>Meu Historico</button>
        <button onClick={() => setSubAba('jogadores')} className={cn('flex-1 py-2 rounded-lg text-sm font-bold transition-all', subAba === 'jogadores' ? 'bg-purple-600 text-white' : 'bg-white/5 text-white/50 hover:bg-white/10')}>Jogadores</button>
      </div>
      ...
      {subAba === 'meu' && (
        loadMeu ? <div className="text-center py-8 text-white/30">Carregando...</div> : (
          <div className="space-y-4">
            {meuHist?.ranking && (
              <div className="bg-gradient-to-r from-purple-900/40 to-indigo-900/40 border border-purple-500/20 rounded-xl p-4">
            ...
            {evolucaoLC.length > 0 && (
              <div className="bg-[#0f1c2e] border border-white/8 rounded-xl p-4">
                <h4 className="text-xs font-bold text-white/50 uppercase mb-3 flex items-center gap-1.5"><TrendingUp size={13} className="text-emerald-400" /> Evolucao por Rodada</h4>
                  ...
                  {evolucaoLC.map((e: any, i: number) => {
                    ...
                    return (
                      <div key={i} className="flex items-center gap-2">
                        <span className="text-[10px] text-white/40 w-8 shrink-0 text-right">R{e.numero}</span>
                        <div className="flex-1 h-5 bg-white/5 rounded-full overflow-hidden relative">
                          <div className={cn('h-full rounded-full transition-all', pts >= 0 ? 'bg-gradient-to-r from-emerald-600 to-emerald-400' : 'bg-gradient-to-r from-red-600 to-red-400')} style={{ width: `${Math.max(width, 4)}%` }} />
                          <span className="absolute inset-0 flex items-center justify-center text-[10px] font-bold text-white">{pts.toFixed(1)} pts</span>
                        </div>
                        <span className="text-[10px] text-white/30 w-10 text-right">{e.acumulado}</span>
                      </div>
                    );
                  })}
                </div>
            ...
            {evolucaoLC.length > 0 && (
              <div className="bg-gradient-to-r from-yellow-900/30 to-amber-900/30 border border-yellow-500/20 rounded-xl p-4">
                  ...
                  {evolucaoLC.map((e: any, i: number) => {
                    ...
                    return (
                      <div key={i} className="flex items-center justify-between text-xs py-1 border-b border-white/5 last:border-0">
                        <span className="text-white/40 w-8">R{e.numero}</span>
                        <span className="text-yellow-400 font-bold">{lc.toFixed(1)} <LendaCoin size={10} className="inline" /></span>
                        <span className={cn('font-bold', diff > 0 ? 'text-emerald-400' : diff < 0 ? 'text-red-400' : 'text-white/30')}>{diff > 0 ? '+' : ''}{diff.toFixed(1)}</span>
              ...
              <div className="bg-gradient-to-r from-cyan-900/30 to-blue-900/30 border border-cyan-500/20 rounded-xl p-4">
                ...
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div className="text-center flex-1">
                    {(() => {
                      ...
                      return (
                        <>
                          <p className={cn('text-lg font-black', diff > 0 ? 'text-emerald-400' : diff < 0 ? 'text-red-400' : 'text-white/50')}>{diff > 0 ? '+' : ''}{diff.toFixed(2)}</p>
                ...
                <div className="space-y-1">
                  {meuHist.minha_valorizacao.map((v: any, i: number) => (
                    <div key={i} className="flex items-center justify-between text-xs py-1 border-b border-white/5 last:border-0">
                      <span className="text-white/40">Rodada {v.numero}</span>
                      <span className="text-white font-bold">{parseFloat(v.preco).toFixed(2)} <LendaCoin size={10} className="inline" /></span>
                      <span className={cn('font-bold', parseFloat(v.variacao) > 0 ? 'text-emerald-400' : parseFloat(v.variacao) < 0 ? 'text-red-400' : 'text-white/30')}>{parseFloat(v.variacao) > 0 ? '+' : ''}{parseFloat(v.variacao).toFixed(2)}</span>
                      <span className="text-purple-400">{parseFloat(v.pontos_rodada).toFixed(1)} pts</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        loadJog ? <div className="text-center py-8 text-white/30">Carregando...</div> : (
          <div className="space-y-3">
            <div className="relative">
              ...
              <input value={busca} onChange={e => setBusca(e.target.value)} placeholder="Buscar jogador..." className="w-full bg-white/5 border border-white/10 rounded-lg pl-9 pr-3 py-2 text-sm text-white placeholder:text-white/30 focus:outline-none focus:border-purple-500/50" />
            </div>
            {jogadoresFiltrados.map((j: any) => {
              const expanded = jogadorExpandido === j.id;
              const valColor = j.valorizacao_total > 0 ? 'text-emerald-400' : j.valorizacao_total < 0 ? 'text-red-400' : 'text-white/40';
              return (
                <div key={j.id} className="bg-[#0f1c2e] border border-white/8 rounded-xl overflow-hidden">
                  <button onClick={() => setJogadorExpandido(expanded ? null : j.id)} className="w-full flex items-center gap-3 p-3 hover:bg-white/5 transition-colors">
                    <Avatar src={j.foto_url ?? j.avatar_url} nome={j.nome} size={10} />
                    <div className="flex-1 min-w-0 text-left">
                      <p className="font-bold text-white text-sm truncate">{j.nome}</p>
                      <p className="text-[10px] text-white/30">{j.posicao} · {j.rodadas?.length ?? 0} rodadas</p>
                    </div>
                    <div className="text-right shrink-0">
                      <p className="text-sm font-black text-white">{j.preco_atual?.toFixed(2)} <LendaCoin size={11} className="inline" /></p>
                      <p className={cn('text-xs font-bold', valColor)}>{j.valorizacao_total > 0 ? '+' : ''}{j.valorizacao_total?.toFixed(2)}</p>
                    </div>
                    ...
                    <div className="px-3 pb-3 border-t border-white/5">
                      ...
                      <div className="overflow-x-auto">
                        <table className="w-full text-xs">
                          <thead><tr className="text-white/30 border-b border-white/5"><th className="text-left py-1 font-medium">Rodada</th><th className="text-center py-1 font-medium">Preco</th><th className="text-center py-1 font-medium">Var.</th><th className="text-center py-1 font-medium">Pts</th><th className="text-center py-1 font-medium">G</th><th className="text-center py-1 font-medium">A</th></tr></thead>
                          <tbody>
                            {j.rodadas?.map((rd: any, idx: number) => {
                              const rodNum = histJogadores?.rodadas?.find((r: any) => r.id === rd.rodada_id)?.numero ?? idx + 1;
                              return (
                                <tr key={rd.rodada_id} className="border-b border-white/5 last:border-0">
                                  <td className="py-1.5 text-white/50">R{rodNum}</td>
                                  <td className="py-1.5 text-center text-white font-bold">{rd.preco.toFixed(2)}</td>
                                  <td className={cn('py-1.5 text-center font-bold', rd.variacao > 0 ? 'text-emerald-400' : rd.variacao < 0 ? 'text-red-400' : 'text-white/30')}>{rd.variacao > 0 ? '+' : ''}{rd.variacao.toFixed(2)}</td>
                                  <td className="py-1.5 text-center text-purple-400 font-bold">{rd.pontos_rodada.toFixed(1)}</td>
